...
export default function Header() {
  ...
  const activeSection = useScrollSpy(
    ["about", "experience", "projects", "hackathons", "data"],
    150
  );
      ...
      <div className="mx-auto max-w-6xl px-5 md:px-8 py-4 flex items-center justify-center md:justify-between">
        ...
        <nav className="flex items-center gap-2">
          ...
          <Button
            ...
            asChild
            variant={
              pathname === "/" && activeSection === "hackathons"
                ? "selected"
                : "outline"
            }
            className="select-none"
            onClick={() => umami.event("Header.Hackathons")}
          >
            <Link href="/#hackathons">
              {isDesktop ? t("hackathons") : <FileCode2 className="h-6 w-6" />}
            </Link>
          </Button>
          <Button
            size={isDesktop ? "sm" : "icon"}
